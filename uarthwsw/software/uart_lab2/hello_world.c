...
#define BAUDRATE 2400
#define ever ;;
#define PULSE() IOWR_ALTERA_AVALON_PIO_DATA(NIOS_HEADER_CONN_BASE,1); for(int i=0; i<10; i++){}; IOWR_ALTERA_AVALON_PIO_DATA(NIOS_HEADER_CONN_BASE,0)
...
#define BAUDRATE BAUDRATE_2 // change this to BAUDRATE_2
#define ever ;;
...
#include "sys/alt_timestamp.h"
#include "altera_avalon_pio_regs.h"
#define WAITSOMETIME() t = alt_timestamp(); while(alt_timestamp()-t < nticks) { /* Do nothing */ }
...
int main()
{
  ...
  int clock_frequency = alt_timestamp_freq();
  uint t;
  uint nticks = clock_frequency * 1.5; // 1.5 sec
  alt_timestamp_start();
  *DIVISOR = (clock_frequency / BAUDRATE) - 1; // set baudrate
